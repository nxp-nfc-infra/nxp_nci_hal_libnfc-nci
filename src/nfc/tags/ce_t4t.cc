...
static bool ce_t4t_send_status(uint16_t status) {
  ...
  LOG(VERBOSE) << StringPrintf("Status:0x%04X", status);
...
static bool ce_t4t_select_file(uint16_t file_id) {
  ...
  LOG(VERBOSE) << StringPrintf("FileID:0x%04X", file_id);

  if (file_id == T4T_CC_FILE_ID) {
    LOG(VERBOSE) << StringPrintf("Select CC file");
...
static bool ce_t4t_update_binary(uint16_t offset, uint8_t length,
  ...
  if ((starting_offset == 0) && (p_t4t->nlen > 0)) {
    ...
    if (ce_cb.p_cback) {
      ...
      LOG(VERBOSE) << StringPrintf("Sent CE_T4T_NDEF_UPDATE_CPLT_EVT");
    }
...
static void ce_t4t_set_version_in_cc(uint8_t version) {
  ...
  LOG(VERBOSE) << StringPrintf("version = 0x%02X", version);
  ...
  */
  ...
  if (ce_cb.mem.t4t.selected_aid_idx < CE_T4T_MAX_REG_AID) {
    ...
    if ((*p_cmd) == 0x00) {
      ...
      LOG(VERBOSE) << StringPrintf("Not found matched AID");
      status_words = T4T_RSP_NOT_FOUND;
    }
  ...
  if (status_words) {
    ...
    if (status_words == T4T_RSP_CMD_CMPLTED) {
      ...
      LOG(VERBOSE) << StringPrintf("T4T CE App selected");
    }
...
void ce_t4t_process_timeout(TIMER_LIST_ENT* p_tle) {
  ...
  LOG(VERBOSE) << StringPrintf("event=%d", p_tle->event);
...
static void ce_t4t_data_cback(uint8_t conn_id, tNFC_CONN_EVT event,
  ...
  if (ce_cb.mem.t4t.status & CE_T4T_STATUS_REG_AID_SELECTED) {
    LOG(VERBOSE) << StringPrintf("CET4T: Forward raw frame to registered AID");
    ...
    LOG(VERBOSE) << StringPrintf(
        "CET4T: Forward raw frame to wildcard AID handler");
...
tNFC_STATUS CE_T4tSetLocalNDEFMsg(bool read_only, uint16_t ndef_msg_max,
  ...
  LOG(VERBOSE) << StringPrintf("read_only=%d, ndef_msg_max=%d, ndef_msg_len=%d",
                             read_only, ndef_msg_max, ndef_msg_len);

  if (!p_ndef_msg) {
    p_t4t->p_ndef_msg = nullptr;

    LOG(VERBOSE) << StringPrintf("T4T is disabled");
    return NFC_STATUS_OK;
  }
...
tCE_T4T_AID_HANDLE CE_T4tRegisterAID(uint8_t aid_len, uint8_t* p_aid,
  ...
  if (aid_len == 0) {
    LOG(VERBOSE) << StringPrintf("registering callback for wildcard AID ");
    ...
    LOG(VERBOSE) << StringPrintf("handle 0x%02x registered (for wildcard AID)",
                               CE_T4T_WILDCARD_AID_HANDLE);
    p_t4t->p_wildcard_aid_cback = p_cback;
    return CE_T4T_WILDCARD_AID_HANDLE;
  }

  LOG(VERBOSE) << StringPrintf("AID [%02X%02X%02X%02X...], %d bytes", *p_aid,
  ...
  if (xx >= CE_T4T_MAX_REG_AID) {
    ...
    LOG(VERBOSE) << StringPrintf("handle 0x%02x registered", xx);
  }
...
extern void CE_T4tDeregisterAID(tCE_T4T_AID_HANDLE aid_handle) {
  ...
  LOG(VERBOSE) << StringPrintf("handle 0x%02x", aid_handle);
